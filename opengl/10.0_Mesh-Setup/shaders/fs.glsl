...
void getFlashLightValues(in SpotLight light, in vec3 norm, in vec3 lightDirection, out vec3 ambient, out vec3 diffuse, out vec3 specular) {
  ...
  if(theta > light.outerCutOff) {
    ...
    float spec = pow(max(dot(cameraDirection, reflectDir), 0.0), material.shine);
    specular = light.attributes.specular * spec * texture(material.specular, TexCoords).rgb;
...
void main() {
  ...
  vec3 spotLightDirection = normalize(spotLight.position - fragPosition);

  vec3 spotAmbient, spotDiffuse, spotSpecular;
  getFlashLightValues(spotLight, norm, spotLightDirection, spotAmbient, spotDiffuse, spotSpecular);

  FragColor = vec4((directionalAmbient + directionalDiffuse + directionalSpecular
                  + pointAmbient + pointDiffuse + pointSpecular
                  + spotAmbient + spotDiffuse + spotSpecular), 1.0);
}
